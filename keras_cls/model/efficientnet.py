...
class EfficientNet():
    def __init__(self, classes=10, type='B0', concat_max_and_average_pool=False, weights='imagenet', loss='ce',
                 dropout=0.1):
        self.type = type
        self.classes = classes
        self.concat_max_and_average_pool = concat_max_and_average_pool
        self.efficientnet_input_size = {'B0': 224, 'B1': 240, 'B2': 260, 'B3': 300, 'B4': 380, 'B5': 456, 'B6': 528,
                                        'B7': 600}
    ...
    def get_model(self, include_top=True):
        ...
        if self.type == 'B0':
            try:
                ...
                try:
                    x = tf.keras.applications.efficientnet.EfficientNetB0(include_top=False, weights=self.weights)(
                        input_layer)
                except:
                    x = EfficientNetB0(include_top=False, weights=self.weights)(input_layer)
        elif self.type == 'B1':
            try:
                x = tf.keras.applications.EfficientNetB1(include_top=False, weights=self.weights)(input_layer)
            except:
                try:
                    x = tf.keras.applications.efficientnet.EfficientNetB1(include_top=False, weights=self.weights)(
                        input_layer)
                except:
                    x = EfficientNetB1(include_top=False, weights=self.weights)(input_layer)
        elif self.type == 'B2':
            try:
                x = tf.keras.applications.EfficientNetB2(include_top=False, weights=self.weights)(input_layer)
            except:
                try:
                    x = tf.keras.applications.efficientnet.EfficientNetB2(include_top=False, weights=self.weights)(
                        input_layer)
                except:
                    x = EfficientNetB2(include_top=False, weights=self.weights)(input_layer)
        elif self.type == 'B3':
            try:
                x = tf.keras.applications.EfficientNetB3(include_top=False, weights=self.weights)(input_layer)
            except:
                try:
                    x = tf.keras.applications.efficientnet.EfficientNetB3(include_top=False, weights=self.weights)(
                        input_layer)
                except:
                    x = EfficientNetB3(include_top=False, weights=self.weights)(input_layer)
        elif self.type == 'B4':
            try:
                x = EfficientNetB4(include_top=False, weights=self.weights)(input_layer)
            except:
                try:
                    x = tf.keras.applications.EfficientNetB4(include_top=False, weights=self.weights)(input_layer)
                except:
                    x = tf.keras.applications.efficientnet.EfficientNetB4(include_top=False, weights=self.weights)(
                        input_layer)
        elif self.type == 'B5':
            try:
            ...
            try:
                ...
                try:
                    x = tf.keras.applications.efficientnet.EfficientNetB6(include_top=False, weights=self.weights)(
                        input_layer)
                except:
                    x = EfficientNetB6(include_top=False, weights=self.weights)(input_layer)
        elif self.type == 'B7':
            try:
                x = tf.keras.applications.EfficientNetB7(include_top=False, weights=self.weights)(input_layer)
            except:
                try:
                    x = tf.keras.applications.efficientnet.EfficientNetB7(include_top=False, weights=self.weights)(
                        input_layer)
                except:
                    x = EfficientNetB7(include_top=False, weights=self.weights)(input_layer)
