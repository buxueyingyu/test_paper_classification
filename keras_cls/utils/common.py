...
def move_plot_window_to_center(plt):
    ...
    try:
        mngr.window.setGeometry(get_monitors()[0].width // 2 - size[0] // 2,
                                get_monitors()[0].height // 2 - size[1] // 2, size[0], size[1])
    except:
        mngr.window.setGeometry = (
            get_monitors()[0].width // 2 - size[0] // 2, get_monitors()[0].height // 2 - size[1] // 2, size[0], size[1])
...
def get_confusion_matrix(dataset, model, save_dir=None):
    ...
    #
    return show_multi_img(dataset, valid_img_path_list[wrong_pred_mask], valid_label_list[wrong_pred_mask],
                          pred_cls[wrong_pred_mask], pred_scores[wrong_pred_mask], dataset.class_names)
...
def show_multi_img(dataset, img_path_list, label_list, pred_list, pred_scores, class_names):
    ...
    for index, (img_path, label, pred, score) in enumerate(zip(img_path_list, label_list, pred_list, pred_scores)):
        title = prefix + "{}/{} < ".format(str(index + 1), len(img_path_list)) + "label:" + class_names[
            label] + ",pred:" + class_names[pred] + ",score:" + "{:.2f}".format(score) + " >" + "\n" + img_path
