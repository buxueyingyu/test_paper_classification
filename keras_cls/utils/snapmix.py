...
def snapmix(batch_imgs, batch_one_hot_labels, last_conv_model, last_dense_layer, img_size, beta):
    ...
    weight1 = 1. - np.sum(car_cam[:, box1[1]:box1[3], box1[0]:box1[2]], axis=(1, 2)) / (
                np.sum(car_cam, axis=(1, 2)) + 1e-8)
    weight2 = np.sum(car_cam[random_batch_indexs, box2[1]:box2[3], box2[0]:box2[2]], axis=(1, 2)) / (
                np.sum(car_cam[random_batch_indexs], axis=(1, 2)) + 1e-8)
    ...
    for bi in range(num_batch):
        batch_imgs[:, box1[1]:box1[3], box1[0]:box1[2]][bi] = \
            cv2.resize(batch_imgs_copy[random_batch_indexs, box2[1]:box2[3], box2[0]:box2[2]][bi],
                       (box1[2] - box1[0], box1[3] - box1[1]), interpolation=cv2.INTER_LINEAR)
